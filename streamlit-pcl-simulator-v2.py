...
def compute_swap(x1, y1, dx, A, gamma, mid_fee, out_fee, fee_gamma):
    ...
    # Calculate the fee
    imbalance = abs(x1 - y1) / (x1 + y1)
    fee_factor = 1 + (out_fee / mid_fee - 1) * (1 - (1 / (1 + imbalance * fee_gamma)))
    fee = mid_fee * fee_factor
    
    dy_after_fee = dy * (1 - fee)
    return dy_after_fee

def calculate_trade_impact(x, y, dx, A, gamma, mid_fee, out_fee, fee_gamma):
    dy = compute_swap(x, y, dx, A, gamma, mid_fee, out_fee, fee_gamma)
    expected_dy = dx * (y / x)
...
gamma = st.sidebar.number_input('Gamma', 0.0, 1.0, 0.01, step=0.001, format="%.3f")
mid_fee = st.sidebar.number_input('Mid Fee', 0.0, 0.1, 0.0003, step=0.0001, format="%.4f")
out_fee = st.sidebar.number_input('Out Fee', 0.0, 0.1, 0.0045, step=0.0001, format="%.4f")
fee_gamma = st.sidebar.number_input('Fee Gamma', 0.0, 1.0, 0.3, step=0.01, format="%.2f")
...
for x in x_range:
    ...
    impact = calculate_trade_impact(Decimal(str(x)), Decimal(str(y)), sale_amount_decimal, A, gamma, mid_fee, out_fee, fee_gamma)
...
balanced_impact = calculate_trade_impact(
    Decimal(str(initial_liquidity_float / 2)),
    Decimal(str(initial_liquidity_float / 2)),
    sale_amount_decimal, A, gamma, mid_fee, out_fee, fee_gamma
)
